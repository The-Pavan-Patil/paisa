...
import { loadInvestmentKindTotalsInr } from "@/lib/queries/investmentAggregates";
...
export async function DashboardDeferred({ month }: { month: string }) {
  ...
  const [trend, { data: expenses }, portfolioDonut, pendingRes, { data: categories }] = await Promise.all([
    loadTwelveMonthTrend(supabase, { userId: user.id, endMonth: month }),
    supabase
      .from("expense_entries")
      .select("amount_paise, category_id")
      .eq("user_id", user.id)
      .gte("month", start)
      .lte("month", end),
    loadInvestmentKindTotalsInr(supabase, user.id),
    supabase
      ...
      <div className="grid gap-3 md:grid-cols-3">
        <Card>
          <CardHeader className="pb-2">
            <CardTitle className="text-xs font-medium text-zinc-600">Pending imports</CardTitle>
          </CardHeader>
          <CardContent className="text-sm font-semibold">{String(pendingRes.count ?? 0)}</CardContent>
        </Card>
      </div>
